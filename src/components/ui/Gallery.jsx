...
import { Row, Col, Card, Popconfirm, message } from 'antd';
...
import delImgIcon from '../../style/imgs/delete.png';
...
class Gallery extends React.Component {
    ...
    componentWillReceiveProps(nextProps) {
        const { imgslist: nextAuth = {} } = nextProps;
        if (nextAuth.data && nextAuth.data.imgs) {
            this.setState({imgs: nextAuth.data.imgs});
        }
    }
    ...
    confirm = (v2) =>{
      ...
      const { fetchData } = this.props;
      fetchData({funcName: 'delImgRequest',params: {'img-name':v2}, stateName: 'delImgRequest'});
      setTimeout(() => {
            fetchData({funcName: 'imgslist',params: {}, stateName: 'imgslist'});
        }, 500);
        message.info('删除成功');
        ...
        const imgsTag = imgs.map(v1 => (
            v1.map(v2 => (
                <div className="gutter-box" key={v2}>
                    <Card className={ this.state.delImg === v2 ?"animated swing hinge" :"animated bounceIn" }
                          bordered={false} bodyStyle={{ padding: 0 }}>
                        <div className="imgDiv">
                            <img onClick={() => this.openGallery(v2)} alt="example" width="100%" src={v2} />
                            <Popconfirm placement="bottomRight" title={text} onConfirm={() => this.confirm(v2)} okText="Yes" cancelText="No">
                                        <img src={delImgIcon} className="delete" />
                            </Popconfirm>
                        </div>
                        <div className="pa-m">
                            <h3>你在我心中是最美</h3>
                            <small><a href={v2} target="_blank" rel="noopener noreferrer">每个微笑都让人沉醉</a></small>
                        </div>
                    </Card>
                </div>
            ))
